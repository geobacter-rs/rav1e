...
pub(crate) mod native {
  ...
  pub fn get_sad<T: Pixel>(
    ...
    _bit_depth: usize, cpu: CpuFeatureLevel,
  ) -> u32 {
    use rkernels::dist::sad::dispatch;
    ...
    let porg_stride = porg.plane_cfg.stride as i32;
    let pref_stride = pref.plane_cfg.stride as i32;

    let r = unsafe {
      dispatch(porg.data_ptr(), porg_stride,
               pref.data_ptr(), pref_stride,
               width as _, height as _,
               bsize, cpu)
        .expect("missing sad kernel")
    };

    #[cfg(feature = "check_asm")] {
      let expected = get_sad_ref(porg, pref,
                                 bsize, _bit_depth,
                                 cpu);
  ...
  pub fn get_satd<T: Pixel>(
    ...
    use rkernels::dist::satd::dispatch;
    ...
    let r = unsafe {
      dispatch(porg.data_ptr(), porg_stride,
               ...
               bsize, cpu)
        .expect("missing satd kernel")
    };

    #[cfg(feature = "check_asm")] {
      let expected = get_satd_ref(porg, pref,
                                  bsize, _bit_depth,
                                  cpu);
      assert_eq!(
        expected, r,
